...
export async function generateMetadata({ params }) {
  ...
  const { data: conseil } = await supabase
    .from('conseils')
    .select('question_ar, answer_ar, specialties(name_fr)')
    .eq('slug', slug)
    .single()
  ...
  return {
    title: `${conseil.question_ar} | دليل الأطباء`,
    description: conseil.answer_ar?.substring(0, 160),
...
export default async function ConseilArPage({ params }) {
  ...
  const { data: doctors } = await supabase
    .from('doctors')
    .select('id, name_fr, slug, rating, wilayas(name_fr)')
  ...
  return (
    <main className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="max-w-5xl mx-auto px-4 py-3 text-sm text-gray-400 flex gap-2 items-center flex-wrap">
        ...
        <span>›</span>
        <Link href="/ar/conseils" className="hover:text-blue-600">نصائح طبية</Link>
        <span>›</span>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-blue-600 rounded-2xl p-6 text-white">
            <h3 className="font-bold text-lg mb-2">تحتاج طبيباً ؟</h3>
            <p className="text-blue-100 text-sm mb-4">ابحث عن أفضل متخصص بالقرب منك</p>
