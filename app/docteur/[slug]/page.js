...
// ✅ cache() sur les services : évite la double requête entre generateMetadata() et DoctorPage()
// Les deux appelaient Supabase séparément pour la même specialty_id.
const getServicesBySpecialty = cache(async (specialtyId) => {
...
export async function generateMetadata({ params }) {
  ...
  // ✅ getServicesBySpecialty (cached) → zéro requête supplémentaire si DoctorPage a déjà appelé cette fonction
  const services = await getServicesBySpecialty(doctor.specialty_id)
...
if (!doctor) {
  // ✅ Vérification dans la table des redirections d'anciens slugs
  ...
  if (slugRedirect) {
    const destination = slugRedirect.new_slug
      ? `/docteur/${slugRedirect.new_slug}`
      : `/specialites/${slugRedirect.specialty_slug}/${slugRedirect.wilaya_slug}`

    // ✅ redirect() importé statiquement en haut du fichier (plus d'import dynamique)
  ...
  // getServicesBySpecialty utilise cache() → zéro requête si generateMetadata a déjà appelé cette fonction
  const [services, { data: similar }] = await Promise.all([
    getServicesBySpecialty(doctor.specialty_id),
    supabase
      .from('doctors')
        ...
        mainEntity: [
          {
            ...
            acceptedAnswer: { '@type': 'Answer', text: `Appelez directement au ${doctor.phone || 'numéro disponible'}.` }
          },
          {
            '@type': 'Question',
            name: `Où se trouve ${doctor.name_fr} ?`,
            acceptedAnswer: { '@type': 'Answer', text: `${doctor.name_fr} est situé à ${doctor.wilayas?.name_fr}${doctor.address ? `, ${doctor.address}` : ''}.` }
          },
        ]
      }
    ]
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      />
      <DoctorTracker doctorId={doctor.id} />

      <header style={{ backgroundColor: '#1A87D8' }} className="py-4 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 flex justify-between items-center">
          {/* ✅ Bug #19 : <Logo> avec <Image> Next.js → lazy loading + pas de CLS */}
          <Logo priority />
          <div className="flex items-center gap-4">
            <Link 
              href={`/ar/docteur/${slug}`} 
        ...
        <div className="lg:col-span-2 space-y-5">
              ...
              {doctor.phone && (
                <div className="mt-5 space-y-3 lg:hidden">
                  {doctor.booking_url && (
                    ...
                    >
                      <span>Prendre RDV en ligne (Confirmé en 1 clic)</span>
                    </a>
                  )}
                  <div className="flex gap-3">
                    <CallButton doctorId={doctor.id} phone={doctor.phone} doctorName={doctor.name_fr}
                      style={{ backgroundColor: '#1E293B' }}
                      className="flex-1 flex items-center justify-center gap-2 hover:opacity-90 text-white py-3 rounded-xl font-semibold transition text-sm">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                      </svg>
                      Appeler
                    </CallButton>
                    {/* ✅ Conditionné sur whatsappNumber (pas doctor.phone) pour éviter href="wa.me/undefined" */}
                    {whatsappNumber && <WhatsappButton doctorId={doctor.id} whatsappNumber={whatsappNumber} doctorName={doctor.name_fr}
          ...
          {(doctor.google_map_url || (doctor.latitude && doctor.longitude)) && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <MapButton
                doctorId={doctor.id}
                mapUrl={doctor.google_map_url || `https://maps.google.com/maps?q=${doctor.latitude},${doctor.longitude}`}
                doctorName={doctor.name_fr}
              ...
              >
                ...
                <div className="absolute inset-0 z-10 bg-slate-900/5 group-hover:bg-slate-900/20 transition-colors duration-300 flex items-center justify-center">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 bg-white/95 backdrop-blur-sm py-2.5 px-4 rounded-xl shadow-md border border-gray-100 flex items-center justify-between transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-bold text-gray-800 truncate">
                        {doctor.address || "Voir l'itinéraire"}
                      </p>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-bold text-gray-900 flex items-center gap-2">
                  ...
                  {doctor.specialties?.name_fr} à {doctor.wilayas?.name_fr}
                </h2>
  ...
  <div className="space-y-3 text-sm text-gray-600 leading-relaxed">
    <p>
      {/* ✅ Bug #21 : getArticle() remplace l'heurístique .includes('e') qui était fausse
          Ex: "Généraliste" contient 'e' mais c'est UN généraliste, pas UNE */}
      {doctor.name_fr} est {getArticle(doctor.specialties?.name_fr)} {doctor.specialties?.name_fr?.toLowerCase()}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sticky top-4">
            ...
            {doctor.booking_url && (
              <a 
                ...
                className="flex flex-col items-center justify-center gap-1 w-full hover:opacity-90 text-white py-3 rounded-xl font-bold transition mb-3 shadow-md"
              >
                ...
                <span className="text-[10.5px] text-blue-100 font-normal">Confirmation instantanée en 1 clic</span>
              </a>
            )}
            {doctor.phone && (
              <CallButton doctorId={doctor.id} phone={doctor.phone} doctorName={doctor.name_fr}
                style={{ backgroundColor: '#1E293B' }}
                className="flex items-center justify-center gap-2 w-full hover:opacity-90 text-white py-3 rounded-xl font-semibold transition mb-3">
            ...
            {/* ✅ Conditionné sur whatsappNumber (pas doctor.phone) pour éviter href="wa.me/undefined" */}
            {whatsappNumber && (
              <WhatsappButton doctorId={doctor.id} whatsappNumber={whatsappNumber} doctorName={doctor.name_fr}
      ...
      <footer style={{ backgroundColor: '#0f172a' }} className="text-gray-400 py-16 border-t border-gray-800 mt-12">
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12 text-left">
            ...
            {/* Colonne 1: À Propos */}
            <div className="space-y-4">
              {/* ✅ Bug #19 : <Logo> avec <Image> Next.js dans le footer */}
              <Logo dark />
