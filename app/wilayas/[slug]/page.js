...
export default async function WilayaPage({ params, searchParams }) {
  ...
  const { data: specialties } = await supabase
    .from('specialties').select('name_fr, slug').order('name_fr')

  const total = totalDoctors || 0
  const totalPages = Math.ceil(total / pageSize)

  const buildUrl = (page) => `/wilayas/${slug}?page=${page}`
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-3 text-sm text-gray-500 flex gap-2 flex-wrap">
        ...
        <span>Wilayas</span>
        <span>›</span>
        <span className="text-gray-800">{wilaya.name_fr}</span>
      </div>

      <div className="bg-blue-700 text-white py-10 px-4">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-3xl font-bold">Medecins a {wilaya.name_fr}</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <p className="text-gray-500 text-sm mb-3">Filtrer par specialite:</p>
                  ...
                  <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition border border-gray-100 p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-blue-600 text-sm">{doctor.specialties?.name_fr}</p>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm mt-8">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Trouver un medecin a {wilaya.name_fr}</h2>
          <p className="text-gray-600 mb-4">
            Dalil Atibaa vous aide a trouver rapidement un medecin a {wilaya.name_fr}.
            Notre annuaire recense les medecins generalistes, dentistes, cardiologues et bien d autres specialistes.
          </p>
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded-xl p-4">
              <p className="font-medium text-gray-700 mb-1">Comment prendre rendez-vous a {wilaya.name_fr} ?</p>
              <p className="text-gray-500 text-sm">Trouvez le medecin et appelez directement au numero indique.</p>
            </div>
            <div className="bg-gray-50 rounded-xl p-4">
              <p className="font-medium text-gray-700 mb-1">Quelles specialites sont disponibles ?</p>
              <p className="text-gray-500 text-sm">Medecine generale, dentisterie, cardiologie, gynecologie et plus.</p>
            </div>
          </div>
        </div>
      </div>
